...
def write_file(filename):
    ...
    with open(filename, 'r+', encoding = 'UTF-8') as f:
        find_list = [line for line in f if find_word in line]
        if len(find_list) != 0:
            print("Такой контакт уже существует!")
            return 0
        else:
            f.write(f"{input_fio} | {input_tel}\n")
...
def replace_list(filename, change_find_list, selection_change = 0):
    ...
    with open ('tell_list.txt', 'w', encoding = 'UTF-8') as f:
        f.write(new_data)
...
def menu():
    file_tel = "phone_book.txt"
    with open(file_tel, 'a', encoding = 'UTF-8') as f:
        f.write('')
        
    while True:
        print("Выберите действие: ")
        print("1 - вывести информацию")
        print("2 - добавить данные в тел. книгу")
        print("3 - для поиска по фамилии или телефону")
        print("4 - изменить контакт")
        print("5 - удалить контакт")
        print("0 - выход из программы")
        number_options = int(input("выбор: "))
        if number_options == 0:
            os.system("cls")
            break

        elif number_options == 1:
            read_file('phone_book.txt')


        elif number_options == 2:
            os.system("cls")
            write_file('phone_book.txt')

        elif number_options == 3:
            os.system("cls")
            find_list = find_file('phone_book.txt')
            search_processing(find_list)

        elif number_options == 4:
            os.system("cls")
            change_file('phone_book.txt')

        elif number_options == 5:
            os.system("cls")
            delete_data('phone_book.txt')
